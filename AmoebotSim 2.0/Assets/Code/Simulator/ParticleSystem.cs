        ...
        private void DiscoverCircuits(bool sendBeepsAndMessages)
        {
            ...
            foreach (Particle particle in particles)
            {
                ...
                while (queue.Count > 0)
                {
                    ...
                    foreach (SysPartitionSet ps in p.PrevPinConfig.partitionSets)
                    {
                        if (ps.IsEmpty())
                        {
                            continue;
                        ...
                        foreach (Pin _pin in ps.GetPins())
                        {
                            ...
                            if (nbrParts[pinLabel] != null)
                            {
                                ...
                                SysPin nbrPin = nbrParts[pinLabel].PrevPinConfig.pinsGlobal[nbrPinId];
            ...
            foreach (Particle p in particles)
            {
                ...
                if (p.PrevPinConfig.placementModeHead != PSPlacementMode.NONE)
                {
                    switch (p.PrevPinConfig.placementModeHead)
                    {
                        ...
                        case PSPlacementMode.LINE_ROTATED:
                            ...
                            p.gCircuit.CodePositionOverride_LineRotated(p.PrevPinConfig.lineRotationHead - 90f, true);
                            break;
                ...
                if (p.IsExpanded() && p.PrevPinConfig.placementModeTail != PSPlacementMode.NONE)
                {
                    switch (p.PrevPinConfig.placementModeTail)
                    {
                        case PSPlacementMode.LINE:
                        ...
                        case PSPlacementMode.LINE_ROTATED:
                            // Rotation 0 means East in pin configuration API but North in graphics API
                            p.gCircuit.CodePositionOverride_LineRotated(p.PrevPinConfig.lineRotationTail - 90f, false);
                            break;
                        case PSPlacementMode.LLOYD:
                ...
                bool manualPositionHead = p.PrevPinConfig.placementModeHead == PSPlacementMode.MANUAL;
                bool manualPositionTail = p.IsExpanded() && p.PrevPinConfig.placementModeTail == PSPlacementMode.MANUAL;
